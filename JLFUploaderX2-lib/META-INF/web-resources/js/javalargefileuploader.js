...
function JavaLargeFileUploader() {
	var globalServletMapping = "uploaderx2/javaLargeFileUploaderServlet";
	var uploadServletMapping = "uploaderx2/javaLargeFileUploaderAsyncServlet";
	var pendingFiles = new Object();
	var bytesPerChunk;

	javaLargeFileUploaderHost = "../sdevinteg/ServiceCenterDEV/";
		...
		$.get(javaLargeFileUploaderHost + globalServletMapping + "?action=getConfig" + appended, function(data) {
			if (data) {
				...
				if (!jQuery.isEmptyObject(data.pendingFiles)) {
					...
					//	pendingFile.paused = (pendingFile.fileCompletionInBytes<pendingFile.originalFileSizeInBytes);
