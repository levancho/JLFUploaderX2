package com.github.levancho.jsuplaoder.largefile.limiter;
...
{
	private static Log log = LogFactory.getLog(RateLimiterConfigurationManager.class);
...
//	private static final Logger log = LoggerFactory.getLogger(RateLimiterConfigurationManager.class);
	...
	@Value("upx2{uploader.rateLimiterConfiguration.evictionTimeInSeconds:120}")
	...
	// 10mb/s
	@Value("upx2{uploader.ratelimiter.maximumRatePerClientInKiloBytes:10240}")
	private volatile long maximumRatePerClientInKiloBytes;


	// 10mb/s
	@Value("upx2{uploader.ratelimiter.maximumOverAllRateInKiloBytes:10240}")
			...
			if (entityIfPresentWithIdentifier != null) {
				...
				for (Entry<UUID, StaticFileState> lavds : entityIfPresentWithIdentifier.getFileStates().entrySet()) {
					if (!lavds.getValue().getStaticFileStateJson().getCrcedBytes().equals(lavds.getValue().getStaticFileStateJson()
						...
						uploadxListenerPropagator.getPropagator().onClientInactivity(key, clientEvictionTimeInSeconds);
						return;
					}
				}
